...
def bottom_up_tree(at: 'AttackTree', root: 'Node', semi_ring: SemiRing):
    match root.node_type:
        case NodeType.ROOT_OR:
            return semi_ring.or_operator([bottom_up_tree(at, child, semi_ring) for child in root.children])
        case NodeType.ROOT_AND:
            return semi_ring.and_operator(([bottom_up_tree(at, child, semi_ring) for child in root.children]))
        case NodeType.OR:
            return semi_ring.or_operator([bottom_up_tree(at, child, semi_ring) for child in root.children])
        case NodeType.AND:
            return semi_ring.and_operator(([bottom_up_tree(at, child, semi_ring) for child in root.children]))
