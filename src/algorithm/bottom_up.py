...
def bottom_up_tree(at: 'AttackTree', root: 'Node', semi_ring: SemiRing):
    ...
    match root.node_type:
        case NodeType.ROOT_OR:
            result = semi_ring.root_or_operator([bottom_up_tree(at, child, semi_ring) for child in root.children])
        case NodeType.ROOT_AND:
            result = semi_ring.root_and_operator(([bottom_up_tree(at, child, semi_ring) for child in root.children]))
        case NodeType.OR:
