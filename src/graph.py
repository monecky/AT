...
class Vertex(object):
    ...
    def __init__(self, graph: "Graph", vertex_type=None, label=None, color_enum=None):
        """
...
def complement(g: "Graph") -> "Graph":
    result = Graph(g.directed, 0)
    k = 0
    map_self_result = {}
    for i in g.vertices:
        node = Vertex(result, k)
        result.add_vertex(node)
        map_self_result[i] = node
        k += 1
    for i in g.vertices:
        for z in g.vertices:
            if not g.is_adjacent(i, z) or not g.is_adjacent(z, i):
                if i.label != z.label:
                    result.add_edge(Edge(map_self_result[i], map_self_result[z]))
                    # HINT
                    # This line remove for directed graphs.
                    # g.add_edge(Edge(i, z))
    return result
...
class Graph(object):
    ...
    def del_edge(self, edge):  # edge = Edge(u,v)
        """
        Deletes an edge from the graph
        :param edge:
        :return: graph
        """
        if edge.tail in self._v and edge.head in self._v:
            self._e.remove(edge)  # this deletes the edge from the edges list
            edge.tail._incidence[edge.head].remove(edge)  # this deletes the edge from the incidence list of the tail
            edge.head._incidence[edge.tail].remove(edge)  # this deletes the edge from the incidence list of the head

    def del_vertex(self, vertex):
        """"
        Deletes a vertex from the graph
        :param vertex: the vertex that we want to delete
        :return graph without the vertex
        """
        for edge in vertex.incidence:  # delete edge from edge list which is incident to the vertex
            self.del_edge(edge)
        self._v.remove(vertex)  # delete vertex from vertex list
        # ToDo: also delete this vertex from the incidence of the neighbours of the vertex
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        result = Graph(self._directed or other.directed, 0)
        k = 0
        map_self_result = {}
        for i in self.vertices:
            node = Vertex(result, k, self.vertices[k].color_enum)
            result.add_vertex(node)
            map_self_result[i] = node
            k += 1
        for i in self.edges:
            result.add_edge(Edge(map_self_result[i.head], map_self_result[i.tail]))
        map_other_self = {}
        j = k
        for i in other.vertices:
            node = Vertex(result, k, other.vertices[k - j].color_enum)
            result.add_vertex(node)
            map_other_self[i] = node
            k += 1
        for i in other.edges:
            result.add_edge(Edge(map_other_self[i.head], map_other_self[i.tail]))
        return result

    def copy(self):
        """
        Make a copy of the graph.
        :param self: is the graph to copy.
        :return: New graph object.
        """
        result = Graph(self._directed, 0)
        k = 0
        map_self_result = {}
        for i in self.vertices:
            node = Vertex(result, k, self.vertices[k].color_enum)
            if self.vertices[k].identifier_number_of_vertices is not None:
                node.identifier_number_of_vertices = self.vertices[k].identifier_number_of_vertices
            if self.vertices[k].identifier_true_or_false is not None:
                node.identifier_true_or_false = self.vertices[k].identifier_true_or_false
            result.add_vertex(node)
            map_self_result[i] = node
            k += 1
        for i in self.edges:
            result.add_edge(Edge(map_self_result[i.head], map_self_result[i.tail]))
        if self.history_vertex is not None:
            """"Properly copy the vertices"""
            result.history_vertex = []
            for label_number_vertex in self.history_vertex:
                result.history_vertex += [result.vertices[label_number_vertex.label]]
        if self.history_similar is not None:
            result.history_similar = self.history_similar[:]
        if self.history_color is not None:
            result.history_color = self.history_color[:]
